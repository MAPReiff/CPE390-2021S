...
/*
  ...
  doing it in separate parts should still be a very small percentage
        of the effort
        for 40 million elements, there are approximately 40k elements in each
  group So if this is a lot slower, it's not because of the nested loop
*/
...
//    compare the speed of sequential access to the speed of writing out of
//    order
//*/
...
*/
template <typename Func>
void benchmark2(const char msg[],
                Func f,
                uint32_t n,
                uint32_t iterationsPer,
                uint32_t numTrials) {
