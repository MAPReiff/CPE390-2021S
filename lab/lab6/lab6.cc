...
int main() {
  ...
  // take the number 5 000000000000000000000000000000101 and set bit 8 from the
  // right (rightmost bit is position 0)
  ...
  // for the following tests figure out what the answer should be, make sure
  // your code is right!
  cout << setBit(17, 9) << '\n';
  // 8 -> 256 and 9 -> 512; 512+17=529

  /* Same as above but this time clear the bit. This can be done by anding with
   * the inverse
   */
  ...
  */
  ...
  // wipe out the last 3 hex digits of the first number by anding with the
  // second then replace with the numbers 0x123
  answer = replaceBits(0xF0F0A5A5, 0xFFFFF000, 0x123);
  // take the value 0xF0F0A5A5, clears all bits that are zero in 0xFFFFF000,
  // then replace by bits in 0x123
